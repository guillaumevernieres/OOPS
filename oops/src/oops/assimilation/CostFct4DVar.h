...
template <typename MODEL>
void CostFct4DVar<MODEL>::runNL(const CtrlVar_ & xx,
                                PostProcessor<State_> & post) const {
  ASSERT(xx.state().checkStatesNumber(1));
  ASSERT(xx.state()[0].validTime() == windowBegin_);
  State_ zz(xx.state()[0]);
  CostFct_::getModel().forecast(zz, xx.modVar(), windowLength_, post);
...
template <typename MODEL>
void CostFct4DVar<MODEL>::runTLM(const CtrlInc_ & dx,
                                 PostProcessorTL<Increment_> & cost,
                                 PostProcessor<Increment_> post) const {
  ASSERT(dx.state()[0].validTime() == windowBegin_);
  Increment_ dz(dx.state()[0]);
  CostFct_::getTLM().forecastTL(dz, dx.modVar(), windowLength_, post, cost);
  ASSERT(dz.validTime() == windowEnd_);
}
...
void CostFct4DVar<MODEL>::runADJ(CtrlInc_ & dx,
  ...
  CostFct_::getTLM().forecastAD(dx.state()[0], dx.modVar(), windowLength_, post, cost);
  ASSERT(dx.state()[0].validTime() == windowBegin_);
}
