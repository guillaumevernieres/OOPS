...
template <typename MODEL>
void CostFct3DVar<MODEL>::runNL(const CtrlVar_ & xx,
...
template <typename MODEL>
void CostFct3DVar<MODEL>::runTLM(const CtrlInc_ & dx,
                                 PostProcessorTL<Increment_> & cost,
                                 PostProcessor<Increment_> post) const {
  Increment_ dz(dx.state()[0]);
  ASSERT(dz.validTime() == windowHalf_);
  CostFct_::getTLM().forecastTL(dz, dx.modVar(), util::Duration(0), post, cost);
  ASSERT(dz.validTime() == windowHalf_);
}
