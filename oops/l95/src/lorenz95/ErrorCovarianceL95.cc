...
// Gaussian structure function
  const unsigned int nn = resol.npoints();
  size_ = nn/2+1;
  std::vector<double> structFct(nn);
  for (unsigned int jj = 0; jj < nn; ++jj) {
    double zz = rscale_ * std::min(jj, nn-jj);
    structFct[jj] = std::exp(-0.5*zz*zz);
  }
// Go to Fourier space
  Eigen::FFT<double> fft;
  std::vector<std::complex<double> > coefs(size_);
  fft.fwd(coefs, structFct);
...
void ErrorCovarianceL95::multiply(const IncrementL95 & dxin,
  ...
  Eigen::FFT<double> fft;
  std::vector<std::complex<double> > coefs(size_);
  fft.fwd(coefs, dxin.asVector());
  for (unsigned int jj = 0; jj < size_; ++jj) {
    coefs[jj] *= bcoefs_[jj];
  }
  fft.inv(dxout.asVector(), coefs);
...
void ErrorCovarianceL95::randomize(IncrementL95 & dx) const {
  ...
  Eigen::FFT<double> fft;
  std::vector<std::complex<double> > coefs(size_);
  fft.fwd(coefs, dx.asVector());
  for (unsigned int jj = 0; jj < size_; ++jj) {
    coefs[jj] *= std::sqrt(bcoefs_[jj]);
  }
  fft.inv(dx.asVector(), coefs);
  dx *= sigmab_;
}
// -----------------------------------------------------------------------------
