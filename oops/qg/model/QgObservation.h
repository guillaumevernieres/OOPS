...
#ifndef QG_MODEL_QGOBSERVATION_H_
#define QG_MODEL_QGOBSERVATION_H_
...
class ObsFactory {
 public:
  static QgObservation * create(ObsSpaceQG &, const eckit::Configuration &);
  virtual ~ObsFactory() { makers_->clear(); }
 protected:
  explicit ObsFactory(const std::string &);
 private:
  virtual QgObservation * make(ObsSpaceQG &, const eckit::Configuration &) =0;
  static std::map < std::string, ObsFactory * > * makers_;
};

template<class T>
class ObsMaker : public ObsFactory {
  virtual QgObservation * make(ObsSpaceQG & odb, const eckit::Configuration & c) {return new T(odb, c);}
 public:
...
class QgObservation : public util::Printable,
                      private boost::noncopyable {
 public:
  static QgObservation * create(ObsSpaceQG & odb, const eckit::Configuration & conf)
    {return ObsFactory::create(odb, conf);}

  virtual ~QgObservation() {}
...
#endif  // QG_MODEL_QGOBSERVATION_H_
