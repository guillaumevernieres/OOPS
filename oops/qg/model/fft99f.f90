...
SUBROUTINE fft99a(a,work,trigs,inc,jump,n,lot)
  ...
  real(kind=kind_real), intent(in) :: a(n), trigs(n)
  real(kind=kind_real),intent(out) :: work(n)
...
!DIR$ IVDEP
  DO l=1,lot
    work(ja)=2.0_kind_real*a(ia)
    work(ja+1)=-2.0_kind_real*a(ia+inc)
    ia=ia+jump
    ja=ja+nx
  END DO
!
...
SUBROUTINE fft99b(work,a,trigs,inc,jump,n,lot)
  ...
  implicit none
  integer, intent(in) :: inc, jump, n, lot
  real(kind=kind_real),intent(in) :: work(n), trigs(n)
  real(kind=kind_real),intent(out) :: a(n)

  integer :: ia, iabase, ib, ibbase, ink, ja, jabase, jb, jbbase, &
           & k, l, nh, nx
  real(kind=kind_real) :: c, s, scale
!
...
!
!  A(0) AND A(N/2)
  scale=1.0_kind_real/REAL(n,kind_real)
  ia=1
  ib=2
  ja=1
  jb=n*inc+1
!DIR$ IVDEP
  DO l=1,lot
    ...
    a(ja+inc)=0.0_kind_real
    a(jb+inc)=0.0_kind_real
    ia=ia+nx
    ib=ib+nx
    ja=ja+jump
    jb=jb+jump
...
SUBROUTINE fft991(a,work,trigs,ifax,inc,jump,n,lot,ISIGN)
  ...
  real(kind=kind_real), intent(in) :: trigs(n)
  real(kind=kind_real), intent(inout) :: a(n)
  real(kind=kind_real), intent(out) :: work(n)
...
SUBROUTINE set99 (trigs, ifax, n)
  ...
  real(kind=kind_real), intent(out) :: trigs(3*n/2+1)
...
SUBROUTINE fftrig(trigs,n,mode)
  ...
  real(kind=kind_real), intent(out) :: trigs(1)

  integer :: i, imode, l, la, nh, nn
  real(kind=kind_real) :: angle, del, pi

  pi=2.0_kind_real*ASIN(1.0_kind_real)
  imode=IABS(mode)
  nn=n
  IF (imode > 1.AND.imode < 6) nn=n/2
  del=(pi+pi)/REAl(nn,kind_real)
  l=nn+nn
  DO i=1,l,2
    angle=0.5_kind_real*REAL(i-1,kind_real)*del
    trigs(i)=COS(angle)
    trigs(i+1)=SIN(angle)
  END DO
  IF (imode == 1) RETURN
  IF (imode == 8) RETURN
  del=0.5_kind_real*del
  nh=(nn+1)/2
  l=nh+nh
  la=nn+nn
  DO i=1,l,2
  ...
  IF (mode == 5) GO TO 40
  DO i=2,nn
    angle=REAL(i-1,kind_real)*del
    trigs(la+i)=2.0_kind_real*SIN(angle)
  END DO
  RETURN
  40 CONTINUE
  del=0.5_kind_real*del
...
SUBROUTINE vpassm(a,b,c,d,trigs,inc1,inc2,inc3,inc4,lot,n,ifac,la)
  ...
  real(kind=kind_real), intent(in) :: a(n), trigs(n)
  real(kind=kind_real), intent(out) :: b(n), c(n), d(n)

  integer :: i, ia, ib, ic, id, ie, iink, igo, ibase, ijk, &
          &  j, ja, jb, jc, jd, je, jink, jump, jbase, &
          &  k, kb, kc, kd, ke, l, la1, m
  real(kind=kind_real) :: c1, c2, c3, c4, s1, s2, s3, s4

  real(kind=kind_real), parameter :: sin36 = 0.587785252292473_kind_real
  real(kind=kind_real), parameter :: cos36 = 0.809016994374947_kind_real
  real(kind=kind_real), parameter :: sin72 = 0.951056516295154_kind_real
  real(kind=kind_real), parameter :: cos72 = 0.309016994374947_kind_real
  real(kind=kind_real), parameter :: sin60 = 0.866025403784437_kind_real
